...
const menuDeCadastro = () => {
  ...
  switch (menu) {
    case "C":
      ...
      break;
    case "R":
      const busca = readline.question("Digite a fruta que você deseja buscar.").toLowerCase();
      listaFrutas.includes(busca) ? `A ${busca} está na posição ${listaFrutas.indexOf(busca)}` : console.log("A fruta não está cadastrada.");
      break;
    case "U":
      const indice = readline.questionInt("Qual a posição que você quer atualizar.");
      if(listaFrutas[indice] !== undefined) {
        console.log(`A fruta cadastrada na posição ${indice} é a ${listaFrutas[indice]}.`);
        const atualizar = readline.question("")
